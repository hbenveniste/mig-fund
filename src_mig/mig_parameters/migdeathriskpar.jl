using CSV, DataFrames, ExcelFiles
...
migflow = join(migflow, iso3c_fundregion, on = :dest, kind = :inner)
rename!(migflow, :fundregion => :destinationregion)

migflow_reg = by(migflow, [:originregion, :destinationregion], df -> sum(df.flow))
...
migflow_reg = join(migflow_reg, regionsdf, on = [:origin, :destination])
...
migdeath = load(joinpath(@__DIR__, "../input_data/migrant-deaths-by-origin-region.xlsx"), "migrant-deaths-by-origin-region!A1:CC22") |> DataFrame
header = 3
...
deaths_regions = [] ; deaths_regions = append!(deaths_regions, migdeath[1, ind_dr[i]] for i in 1:length(ind_dr))
...
death_journey = by(death_journey, [:origin, :deathloc], df -> sum(df.missing))
...
death_journey = join(death_journey, origin_fund, on = :origin, kind =:left)
rename!(death_journey, :fund => :origin_f)
death_journey = join(death_journey, death_fund, on = :deathloc, kind = :left)
rename!(death_journey, :fund => :deathloc_f)

for i in 1:length(death_journey[:,1])
...
for i in 1:size(death_journey,1)
...
for d in intersect(unique(death_journey[!,:deathloc_f]),regions)
    for (o1, o2) in [("SSA", "Horn of Africa"), ("SSA", "Horn of Africa (P)"), ("CHI", "East Asia (P)")]
        deaths[ind[o1], ind[d]] += death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc_f] == d for i in 1:size(death_journey,1)]),:missing]
    end
    ...
    for (o1, o2) in [(i, j) for i in ["MDE", "SAS"], j in ["Middle East and South Asia", "Middle East and South Asia (P)"]]
        deaths[ind[o1], ind[d]] += iszero(dmde + dsas) != true ? death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc_f] == d for i in 1:size(death_journey,1)]),:missing] * (o1 == "MDE" ? dmde : dsas) / (dmde + dsas) : 0.0
    end
    ...
    for (o1, o2) in [(i, j) for i in ["CAM", "LAM", "SIS"], j in ["Latin America and the Caribbean (P)"]] 
        deaths[ind[o1], ind[d]] += iszero(dcam + dlam + dsis) != true ? death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc_f] == d for i in 1:size(death_journey,1)]),:missing] * (o1 == "CAM" ? dcam : (o1 == "LAM" ? dlam : dsis)) / (dcam + dlam + dsis) : 0.0
    end
end

# We attribute missing migrants of unknown origin proportionally to origins of known missing migrants in the region
for d in intersect(unique(death_journey[!,:deathloc_f]),regions)
    dd = Dict(regions[i] => deaths[i, ind[d]] for i in 1:length(regions))
    for o in regions
        deaths[ind[o], ind[d]] += iszero(sum([dd[regions[i]] for i in 1:length(regions)])) != true ? death_journey[findfirst([death_journey[i,:origin] == "Unknown" && death_journey[i,:deathloc_f] == d for i in 1:size(death_journey,1)]),:missing]  * dd[o] / sum([dd[regions[i]] for i in 1:length(regions)]) : 0.0
    end
end
...
for o in intersect(unique(death_journey[!,:origin_f]),regions)
    deaths[ind[o], ind["SSA"]] += death_journey[findfirst([death_journey[i,:origin_f] == o && death_journey[i,:deathloc] == "Horn of Africa" for i in 1:size(death_journey,1)]),:missing]
    # Assume migrants dead at US-Mexico border were trying to enter the USA
    deaths[ind[o], ind["USA"]] += death_journey[findfirst([death_journey[i,:origin_f] == o && death_journey[i,:deathloc] == "US-Mexico border" for i in 1:size(death_journey,1)]),:missing]
    if o != "EEU" && o != "WEU"
...
for (d1, d2) in [("SSA", "Horn of Africa"), ("USA", "US-Mexico border"), ("EEU", "Eastern Mediterranean"), ("WEU", "Western + Central Mediterranean")]
    for (o1, o2) in [("SSA", "Horn of Africa"), ("SSA", "Horn of Africa (P)"), ("CHI", "East Asia (P)")]
        deaths[ind[o1], ind[d1]] += death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc] == d2 for i in 1:size(death_journey,1)]),:missing]
    end    
    dmde = deaths[ind["MDE"], ind[d1]] ; dsas = deaths[ind["SAS"], ind[d1]]
    for (o1, o2) in [(i, j) for i in ["MDE", "SAS"], j in ["Middle East and South Asia", "Middle East and South Asia (P)"]]
        deaths[ind[o1], ind[d1]] += iszero(dmde + dsas) != true ? death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc] == d2 for i in 1:size(death_journey,1)]),:missing] * (o1 == "MDE" ? dmde : dsas) / (dmde + dsas) : 0.0
    end
    dcam = deaths[ind["CAM"], ind[d1]] ; dlam = deaths[ind["LAM"], ind[d1]] ; dsis = deaths[ind["SIS"], ind[d1]]
    for (o1, o2) in [(i, j) for i in ["CAM", "LAM", "SIS"], j in ["Latin America and the Caribbean (P)"]] 
        deaths[ind[o1], ind[d1]] += iszero(dcam + dlam + dsis) != true ? death_journey[findfirst([death_journey[i,:origin] == o2 && death_journey[i,:deathloc] == d2 for i in 1:size(death_journey,1)]),:missing] * (o1 == "CAM" ? dcam : (o1 == "LAM" ? dlam : dsis)) / (dcam + dlam + dsis) : 0.0
    end 
    dd = Dict(regions[i] => deaths[i, ind[d1]] for i in 1:length(regions))
    for o in regions
        deaths[ind[o], ind[d1]] += iszero(sum([dd[regions[i]] for i in 1:length(regions)])) != true ? death_journey[findfirst([death_journey[i,:origin] == "Unknown" && death_journey[i,:deathloc] == d2 for i in 1:size(death_journey,1)]),:missing] * dd[o] / sum([dd[regions[i]] for i in 1:length(regions)]) : 0.0
    end
end
...
deathrisk = []
for o in 1:length(regions)
    orisk = [iszero(flows[o, j]) ? 0.0 : deaths[o, j] / flows[o, j] for j in 1:length(regions)]
