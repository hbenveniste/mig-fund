...
ssp1 = CSV.read(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP1.csv"))
ssp2 = CSV.read(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP2.csv"))
ssp3 = CSV.read(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP3.csv"))
ssp4 = CSV.read(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP4.csv"))
ssp5 = CSV.read(joinpath(@__DIR__, "../../Documents/YSSP_IIASA/data/pop_samirkc/SSP5.csv"))
...
agegroup_c = by(ssp, [:age, :region, :period, :scen], d -> (inmig = sum(d.inmig), pop = sum(d.pop)))
...
agegroup_c = join(agegroup_c, isonum_fundregion, on = :region, kind = :left)
...
deleterows!(agegroup_c, channelsind)
permutecols!(agegroup_c, [4,3,7,2,1,6,5])

agegroup = by(agegroup_c, [:scen, :period, :fundregion, :age], d -> (pop = sum(d.pop), inmig = sum(d.inmig)))

# Computing shares of migrants by age
ageshare = by(agegroup, [:scen, :fundregion, :period], d -> sum(d.inmig))
rename!(ageshare, :x1 => :inmig_sum)
agegroup = join(agegroup, ageshare, on = [:scen, :fundregion, :period])
agegroup[!,:share] = agegroup[!,:inmig] ./ agegroup[!,:inmig_sum]
for i in 1:size(agegroup,1) ; if agegroup[!,:inmig][i] == 0 && agegroup[!,:inmig_sum][i] == 0 ; agegroup[!,:share][i] = 0 end end

# Sorting the data
regionsdf = DataFrame(fundregion = regions, index = 1:16)
agegroup = join(agegroup, regionsdf, on = :fundregion)
...
age_all = join(age_all, agegroup_simple, on = :age)
age_all[:,:share] ./= 5
