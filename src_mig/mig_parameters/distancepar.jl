...
loc = join(loc, pop2015weight[:,[:country_code, :fundregion, :weight]], on = :country_code)
coordinates = by(loc, :fundregion, d -> (lat = sum(d.Latitude .* d.weight), lon = sum(d.Longitude .* d.weight)))
...
dist[!,:distance] = [haversine(dist[!,:loc_or][i], dist[!,:loc_dest][i], earthradius[1]) for i in 1:size(dist, 1)]

# Sort data
regionsdf = DataFrame(origin = regions, index_or = 1:16)
dist = join(dist, regionsdf, on = :origin)
rename!(regionsdf, :origin => :destination, :index_or => :index_dest)
dist = join(dist, regionsdf, on = :destination)
